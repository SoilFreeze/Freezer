...
def server(input, output, session):
    ...
    proj_df_state = reactive.Value(pd.DataFrame())
    ...
    def current_lookback_days():
        if hasattr(input, "full_data_toggle") and input.full_data_toggle():
            p_meta = project_metadata.get() or {}
    ...
    def page_content():
        ...
        if page in ["Summary", "Data Processing", "Admin Tools"]:
            if page == "Summary":
                return ui.div(summary_ui("summary_module"))
    ...
    time_vs_temp_server("time_vs_temp_module", 
                        client=client, 
                        selected_project=input.selected_project,
                        lookback_days=current_lookback_days,
                        global_show_masked=input.global_show_masked,
                        global_show_baddata=input.global_show_baddata,
                        global_show_map=input.global_show_map,
                        global_show_elevation=input.global_show_elevation,
                        unit_mode=input.unit_toggle,
                        unit_label=current_unit_label,
                        display_tz=current_display_tz,
                        active_refs=current_active_refs,
